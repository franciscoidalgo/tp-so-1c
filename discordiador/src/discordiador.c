...
// Segmentacion
	...
	pthread_create(&un_hilo,NULL, (void*) iniciar_patota_A_SEG,conexion);
	pthread_join(un_hilo,NULL);

	sleep(2);
	conexion = crear_conexion(ip,puerto);
	pthread_create(&un_hilo,NULL, (void*) iniciar_patota_C_SEG,conexion);
	pthread_join(un_hilo,NULL);
	...
	// esperar 10 ciclos de CPU

	sleep(2);
	conexion = crear_conexion(ip,puerto);
	pthread_create(&otro_hilo,NULL, (void*) enviar_ubicacion_del_tripulante_1_de_la_patota_A_1er_movimiento,conexion);
	pthread_join(otro_hilo,NULL);
	
	sleep(2);
	conexion = crear_conexion(ip,puerto);
	pthread_create(&otro_hilo,NULL, (void*) enviar_ubicacion_del_tripulante_1_de_la_patota_A_2do_movimiento,conexion);
	pthread_join(otro_hilo,NULL);
	
	sleep(2);
	conexion = crear_conexion(ip,puerto);
	pthread_create(&otro_hilo,NULL, (void*) enviar_ubicacion_del_tripulante_1_de_la_patota_A_3ro_movimiento,conexion);
	pthread_join(otro_hilo,NULL);
	
	sleep(2);
	conexion = crear_conexion(ip,puerto);
	pthread_create(&otro_hilo,NULL, (void*) enviar_ubicacion_del_tripulante_1_de_la_patota_A_4rto_movimiento,conexion);
	pthread_join(otro_hilo,NULL);


	// Fin esperar 10 ciclos de CPU

	sleep(2);
	conexion = crear_conexion(ip,puerto);
	pthread_create(&otro_hilo,NULL, (void*) expulsar_tripulante_1_patota_A_SEG,conexion);
	pthread_join(otro_hilo,NULL);

	sleep(2);
	conexion = crear_conexion(ip,puerto);
	pthread_create(&otro_hilo,NULL, (void*) expulsar_tripulante_1_patota_C_SEG,conexion);
	pthread_join(otro_hilo,NULL);
	...
	sleep(2);
	conexion = crear_conexion(ip,puerto);
	pthread_create(&otro_hilo,NULL, (void*) iniciar_patota_B_SEG,conexion);
	pthread_join(otro_hilo,NULL);

	sleep(2);
	conexion = crear_conexion(ip,puerto);
	pthread_create(&otro_hilo,NULL, (void*) expulsar_tripulante_2_patota_A_SEG,conexion);
	pthread_join(otro_hilo,NULL);
	
	sleep(2);
	conexion = crear_conexion(ip,puerto);
	pthread_create(&otro_hilo,NULL, (void*) expulsar_tripulante_3_patota_A_SEG,conexion);
	pthread_join(otro_hilo,NULL);
	
	sleep(2);
	conexion = crear_conexion(ip,puerto);
	pthread_create(&otro_hilo,NULL, (void*) expulsar_tripulante_4_patota_A_SEG,conexion);
	pthread_join(otro_hilo,NULL);
	
	sleep(2);
	conexion = crear_conexion(ip,puerto);
	pthread_create(&otro_hilo,NULL, (void*) expulsar_tripulante_1_patota_B_SEG,conexion);
	pthread_join(otro_hilo,NULL);
	
	sleep(2);
	conexion = crear_conexion(ip,puerto);
	pthread_create(&otro_hilo,NULL, (void*) expulsar_tripulante_2_patota_B_SEG,conexion);
	pthread_join(otro_hilo,NULL);
...
// Paginacion
	...
	// sleep(2);
	// conexion = crear_conexion(ip,puerto);
	// pthread_create(&un_hilo,NULL, (void*) iniciar_patota_1_PAG,conexion);
	// pthread_join(un_hilo,NULL);

	// sleep(2);
	// conexion = crear_conexion(ip,puerto);
	// pthread_create(&un_hilo,NULL, (void*) iniciar_patota_2_PAG,conexion);
	// pthread_join(un_hilo,NULL);

	// sleep(2);
	// conexion = crear_conexion(ip,puerto);
	// pthread_create(&un_hilo,NULL, (void*) iniciar_patota_3_PAG,conexion);
	// pthread_join(un_hilo,NULL);


	// sleep(2);
	// conexion = crear_conexion(ip,puerto);
	// pthread_create(&otro_hilo,NULL, (void*) expulsar_tripulante_1_patota_3_PAG,conexion);
	// pthread_join(otro_hilo,NULL);
...
void iniciar_patota_A_SEG(int conexion)
{
	...
	char* pid;
	pid = config_get_string_value(config,"PID_SEGMENTACION_1");
	log_info(logger,pid);
	agregar_a_paquete(paquete,pid,strlen(pid)+1);
	
	char* cant_tripulantes = (char*) config_get_string_value(config,"CANT_TRIPULANTES_SEG_1");
	agregar_a_paquete(paquete,cant_tripulantes,strlen(cant_tripulantes)+1);

	char* tarea,*posiciones;
	
	tarea = config_get_string_value(config,"TAREA_1_COMPLETA_SEG_1");
	log_info(logger,tarea);
	agregar_a_paquete(paquete,tarea,strlen(tarea)+1);

	tarea = config_get_string_value(config,"TAREA_2_COMPLETA_SEG_1");
	log_info(logger,tarea);
	agregar_a_paquete(paquete,tarea,strlen(tarea)+1);
	
	tarea = config_get_string_value(config,"TAREA_3_COMPLETA_SEG_1");
	log_info(logger,tarea);
	agregar_a_paquete(paquete,tarea,strlen(tarea)+1);
	
	tarea = config_get_string_value(config,"TAREA_4_COMPLETA_SEG_1");
	log_info(logger,tarea);
	agregar_a_paquete(paquete,tarea,strlen(tarea)+1);
	
	tarea = config_get_string_value(config,"TAREA_5_COMPLETA_SEG_1");
	log_info(logger,tarea);
	agregar_a_paquete(paquete,tarea,strlen(tarea)+1);
	
	tarea = config_get_string_value(config,"TAREA_6_COMPLETA_SEG_1");
	log_info(logger,tarea);
	agregar_a_paquete(paquete,tarea,strlen(tarea)+1);
	
	tarea = config_get_string_value(config,"TAREA_7_COMPLETA_SEG_1");
	log_info(logger,tarea);
	agregar_a_paquete(paquete,tarea,strlen(tarea)+1);
	
	tarea = config_get_string_value(config,"TAREA_8_COMPLETA_SEG_1");
	log_info(logger,tarea);
	agregar_a_paquete(paquete,tarea,strlen(tarea)+1);
	
	tarea = config_get_string_value(config,"TAREA_9_COMPLETA_SEG_1");
	log_info(logger,tarea);
	agregar_a_paquete(paquete,tarea,strlen(tarea)+1);
	
	tarea = config_get_string_value(config,"TAREA_10_COMPLETA_SEG_1");
	log_info(logger,tarea);
	agregar_a_paquete(paquete,tarea,strlen(tarea)+1);

	posiciones = config_get_string_value(config,"POS_SEG_1");
	log_info(logger,posiciones);
	agregar_a_paquete(paquete,posiciones,strlen(posiciones)+1);
...
void iniciar_patota_B_SEG(int conexion)
{
	...
	char* pid;
	pid = config_get_string_value(config,"PID_SEGMENTACION_2");
	log_info(logger,pid);
	agregar_a_paquete(paquete,pid,strlen(pid)+1);
	
	char* cant_tripulantes = (char*) config_get_string_value(config,"CANT_TRIPULANTES_SEG_2");
	agregar_a_paquete(paquete,cant_tripulantes,strlen(cant_tripulantes)+1);

	char* tarea,*posiciones;
	
	tarea = config_get_string_value(config,"TAREA_1_COMPLETA_SEG_2");
	log_info(logger,tarea);
	agregar_a_paquete(paquete,tarea,strlen(tarea)+1);

	tarea = config_get_string_value(config,"TAREA_2_COMPLETA_SEG_2");
	log_info(logger,tarea);
	agregar_a_paquete(paquete,tarea,strlen(tarea)+1);
	
	tarea = config_get_string_value(config,"TAREA_3_COMPLETA_SEG_2");
	log_info(logger,tarea);
	agregar_a_paquete(paquete,tarea,strlen(tarea)+1);
	
	posiciones = config_get_string_value(config,"POS_SEG_2");
	log_info(logger,posiciones);
	agregar_a_paquete(paquete,posiciones,strlen(posiciones)+1);
...
void iniciar_patota_C_SEG(int conexion)
{
	...
	char* pid;
	pid = config_get_string_value(config,"PID_SEGMENTACION_3");
	log_info(logger,pid);
	agregar_a_paquete(paquete,pid,strlen(pid)+1);
	
	char* cant_tripulantes = (char*) config_get_string_value(config,"CANT_TRIPULANTES_SEG_3");
	agregar_a_paquete(paquete,cant_tripulantes,strlen(cant_tripulantes)+1);

	char* tarea,*posiciones;
	
	tarea = config_get_string_value(config,"TAREA_1_COMPLETA_SEG_3");
	log_info(logger,tarea);
	agregar_a_paquete(paquete,tarea,strlen(tarea)+1);

	posiciones = config_get_string_value(config,"POS_SEG_3");
	log_info(logger,posiciones);
	agregar_a_paquete(paquete,posiciones,strlen(posiciones)+1);
...
void iniciar_patota_1_PAG(int conexion)
{
	...
	char* pid;
	pid = config_get_string_value(config,"PID_PAGINACION_1");
	log_info(logger,pid);
	agregar_a_paquete(paquete,pid,strlen(pid)+1);
	
	char* cant_tripulantes = (char*) config_get_string_value(config,"CANT_TRIPULANTES_PAG_1");
	agregar_a_paquete(paquete,cant_tripulantes,strlen(cant_tripulantes)+1);

	char* tarea,*posiciones;
	
	tarea = config_get_string_value(config,"TAREA_1_COMPLETA_PAG_1");
	log_info(logger,tarea);
	agregar_a_paquete(paquete,tarea,strlen(tarea)+1);

	tarea = config_get_string_value(config,"TAREA_2_COMPLETA_PAG_1");
	log_info(logger,tarea);
	agregar_a_paquete(paquete,tarea,strlen(tarea)+1);
	
	tarea = config_get_string_value(config,"TAREA_3_COMPLETA_PAG_1");
	log_info(logger,tarea);
	agregar_a_paquete(paquete,tarea,strlen(tarea)+1);

	posiciones = config_get_string_value(config,"POS_PAG_1");
	log_info(logger,posiciones);
	agregar_a_paquete(paquete,posiciones,strlen(posiciones)+1);
...
void iniciar_patota_2_PAG(int conexion)
{
	...
	char* pid;
	pid = config_get_string_value(config,"PID_PAGINACION_2");
	log_info(logger,pid);
	agregar_a_paquete(paquete,pid,strlen(pid)+1);
	
	char* cant_tripulantes = (char*) config_get_string_value(config,"CANT_TRIPULANTES_PAG_2");
	agregar_a_paquete(paquete,cant_tripulantes,strlen(cant_tripulantes)+1);
	...
	tarea = config_get_string_value(config,"TAREA_1_COMPLETA_PAG_2");
	log_info(logger,tarea);
	agregar_a_paquete(paquete,tarea,strlen(tarea)+1);
	// free(tarea);

	tarea = config_get_string_value(config,"TAREA_2_COMPLETA_PAG_2");
	log_info(logger,tarea);
	agregar_a_paquete(paquete,tarea,strlen(tarea)+1);
	
	tarea = config_get_string_value(config,"TAREA_3_COMPLETA_PAG_2");
	log_info(logger,tarea);
	agregar_a_paquete(paquete,tarea,strlen(tarea)+1);

	posiciones = config_get_string_value(config,"POS_PAG_2");
	log_info(logger,posiciones);
	agregar_a_paquete(paquete,posiciones,strlen(posiciones)+1);
...
void iniciar_patota_3_PAG(int conexion)
{
	...
	char* pid;
	pid = config_get_string_value(config,"PID_PAGINACION_3");
	log_info(logger,pid);
	agregar_a_paquete(paquete,pid,strlen(pid)+1);
	
	char* cant_tripulantes = (char*) config_get_string_value(config,"CANT_TRIPULANTES_PAG_3");
	agregar_a_paquete(paquete,cant_tripulantes,strlen(cant_tripulantes)+1);
	...
	tarea = config_get_string_value(config,"TAREA_1_COMPLETA_PAG_3");
	log_info(logger,tarea);
	agregar_a_paquete(paquete,tarea,strlen(tarea)+1);
	// free(tarea);

	tarea = config_get_string_value(config,"TAREA_2_COMPLETA_PAG_3");
	log_info(logger,tarea);
	agregar_a_paquete(paquete,tarea,strlen(tarea)+1);
	
	tarea = config_get_string_value(config,"TAREA_3_COMPLETA_PAG_3");
	log_info(logger,tarea);
	agregar_a_paquete(paquete,tarea,strlen(tarea)+1);
	
	tarea = config_get_string_value(config,"TAREA_4_COMPLETA_PAG_3");
	log_info(logger,tarea);
	agregar_a_paquete(paquete,tarea,strlen(tarea)+1);
	
	tarea = config_get_string_value(config,"TAREA_5_COMPLETA_PAG_3");
	log_info(logger,tarea);
	agregar_a_paquete(paquete,tarea,strlen(tarea)+1);
	
	tarea = config_get_string_value(config,"TAREA_6_COMPLETA_PAG_3");
	log_info(logger,tarea);
	agregar_a_paquete(paquete,tarea,strlen(tarea)+1);
	
	tarea = config_get_string_value(config,"TAREA_7_COMPLETA_PAG_3");
	log_info(logger,tarea);
	agregar_a_paquete(paquete,tarea,strlen(tarea)+1);
	
	tarea = config_get_string_value(config,"TAREA_8_COMPLETA_PAG_3");
	log_info(logger,tarea);
	agregar_a_paquete(paquete,tarea,strlen(tarea)+1);
	
	tarea = config_get_string_value(config,"TAREA_9_COMPLETA_PAG_3");
	log_info(logger,tarea);
	agregar_a_paquete(paquete,tarea,strlen(tarea)+1);

	posiciones = config_get_string_value(config,"POS_PAG_3");
	log_info(logger,posiciones);
	agregar_a_paquete(paquete,posiciones,strlen(posiciones)+1);
...
char* enviar_ubicacion_del_tripulante_1_de_la_patota_A_1er_movimiento(int conexion)
{
	...
	estado = config_get_string_value(config,"NUEVA_POS_X_1");
	log_info(logger,estado);
	agregar_a_paquete(paquete,estado,strlen(estado)+1);

	estado = config_get_string_value(config,"NUEVA_POS_Y_1");
	log_info(logger,estado);
	agregar_a_paquete(paquete,estado,strlen(estado)+1);
...
char* enviar_ubicacion_del_tripulante_1_de_la_patota_A_2do_movimiento(int conexion)
{
	...
	estado = config_get_string_value(config,"NUEVA_POS_X_2");
	log_info(logger,estado);
	agregar_a_paquete(paquete,estado,strlen(estado)+1);

	estado = config_get_string_value(config,"NUEVA_POS_Y_2");
	log_info(logger,estado);
	agregar_a_paquete(paquete,estado,strlen(estado)+1);
...
char* enviar_ubicacion_del_tripulante_1_de_la_patota_A_3ro_movimiento(int conexion)
{
	...
	estado = config_get_string_value(config,"NUEVA_POS_X_3");
	log_info(logger,estado);
	agregar_a_paquete(paquete,estado,strlen(estado)+1);

	estado = config_get_string_value(config,"NUEVA_POS_Y_3");
	log_info(logger,estado);
	agregar_a_paquete(paquete,estado,strlen(estado)+1);
...
char* enviar_ubicacion_del_tripulante_1_de_la_patota_A_4rto_movimiento(int conexion)
{
	...
	estado = config_get_string_value(config,"NUEVA_POS_X_4");
	log_info(logger,estado);
	agregar_a_paquete(paquete,estado,strlen(estado)+1);

	estado = config_get_string_value(config,"NUEVA_POS_Y_4");
	log_info(logger,estado);
	agregar_a_paquete(paquete,estado,strlen(estado)+1);
...
char* expulsar_tripulante_4_patota_A_SEG(int conexion)
{
	...
	tid = config_get_string_value(config,"TID_EXP_4");
	log_info(logger,tid);
	agregar_a_paquete(paquete,tid,strlen(tid)+1);
